...
import { EkibColumnProps } from "./EkibColumn";
interface EkibCellProps {
  data: EkibColumnProps;
}

export const EkibCell: React.FC<EkibCellProps> = ({ data }) => {
  const router = useRouter();
  const [open, setOpen] = useState(false);

  const onConfirm = async () => {
    try {
      await axios.delete(`/api/ekib/${data.id}`);
